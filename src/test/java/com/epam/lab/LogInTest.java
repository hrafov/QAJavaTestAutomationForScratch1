...
//TODO  4test & not understandable why its not work without this 2 strings - try to investigate! 
//    	currentURL = DriverManager.getDriver().getCurrentUrl();
//        System.out.println("currentURL from the beginning #1 = " + currentURL);
    	
        loginPageBo.logIn(user.getLogin(), user.getPassword());
        Assert.assertEquals(user.getLogin(), homePageBo.getNameFromProfileButton());        
        homePageBo.logout();
        //TODO am i need assertion here - may be not?
        //Assert.assertEquals(loginPageBO. getStartCrearingButton() .getText()?, "Start Creating");        
    }    
    ...
    public void logInTestWithBlankLoginAndPassword() {
        ...
        //TODO add assersion here like:
//      for (int i = 4; i <= 4; i++ ) {
//      System.out.println("text in div[" + i + "] : " + findWebElementByCSSSelector("form>div:nth-child("+i+")").getText() );        
//      }  
        
        //TODO to constant RETRY_PAGE if any - its often happands when i did it manually 
        //Assert.assertEquals(currentURL, "https://scratch.mit.edu/login_retry/");        
        loginPageBo = new LoginPageBo();        
    }
...
//TODO        
//      System.out.println("text from our errror button - " + 
//      findWebElementByXPath("//*[@id=\"navigation\"]/div/ul/li[8]/div/div/form/div[4]").getText());
        loginPageBo = new LoginPageBo();
